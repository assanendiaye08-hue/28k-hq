 ...
 * TimerTransition
 *
 * Transition card shown between pomodoro phases.
...
const btnPrimary =
  'bg-brand hover:bg-brand-light active:scale-[0.98] text-surface-base font-semibold py-3 px-8 rounded-xl shadow-lg shadow-brand/20 transition-all';
const btnGhost =
  'text-text-tertiary hover:text-error text-sm transition-colors';
      ...
      <div className="flex flex-col items-center gap-6 py-4 animate-fade-in">
        {type === 'work_done' && (
          <>
            <div className="text-center">
              ...
              {isLongBreak && (
                <p className="text-success text-sm mt-1">Time for a long break</p>
              )}
            </div>
            <SessionDots completed={pomodoroCount ?? 0} total={targetSessions ?? null} />
            <button onClick={onStartBreak} className={btnPrimary}>Start Break</button>
            <button onClick={onStop} className={btnGhost}>Stop Timer</button>
          </>
        )}
        ...
        {type === 'break_done' && (
          <>
            ...
            <SessionDots completed={pomodoroCount ?? 0} total={targetSessions ?? null} />
            <button onClick={onStartWork} className={btnPrimary}>Resume Work</button>
            <button onClick={onStop} className={btnGhost}>Stop Timer</button>
          </>
        )}
        ...
        {type === 'session_complete' && (
          <>
            ...
            <button onClick={onDismiss} className={btnPrimary}>Done</button>
          </>
        )}
      </div>
    </Card>
